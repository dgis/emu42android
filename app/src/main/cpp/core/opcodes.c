...
VOID o802(LPBYTE I) // A=IN
{
	...
	// emulate Lewis bug
	if (   (w.pc & 1) == 0
		|| (   !(            mNCE2 && (w.pc & w.NCE2Mask)  == w.NCE2Base)
			&& !(            mNCE3 && (w.pc & w.NCE3Mask)  == w.NCE3Base)
			&& !(w.bSlave && sNCE2 && (w.pc & wS.NCE2Mask) == wS.NCE2Base)
			&& !(w.bSlave && sNCE3 && (w.pc & wS.NCE3Mask) == wS.NCE3Base)
		   )
	   )
		w.pc+=3;
